...
| 20 | Repo layout | One repo (this one), split internally: `/framework` (general-purpose library source) vs. `/examples` (V1 proof-of-concept flow specs) — both proprietary, `UNLICENSED` (see repo-root `LICENSE`), no open-source release planned | Framework embedded inside a service repo; specs and framework mixed with no boundary | Specs are company-specific and shouldn't mix with the general-purpose library code even though neither is being open-sourced — keeping the boundary clean still makes the framework reusable across other internal projects without dragging spec-authoring baggage along. `git init` done, `.gitignore` excludes `research/` (third-party reference clones — must never enter this repo's history), `.env`/`.env.local`, and `.evident/` (local run bundles). |
